...
def authenticate():
    ...
    if(username != user):
        if(password != passw): # both username and password are wrong
              return render_template('error.html', message="Incorrect username and password")
        return render_template('error.html', message="Incorrect username") # username is wrong
    if(password != passw): # password is wrong
